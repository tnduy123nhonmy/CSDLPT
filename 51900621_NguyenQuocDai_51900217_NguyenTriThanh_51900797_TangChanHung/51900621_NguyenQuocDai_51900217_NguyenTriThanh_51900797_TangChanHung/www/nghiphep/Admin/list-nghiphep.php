...
<body>
	...
	<div class="container">
		...
		<a href="/index.php" id="create" class="btn btn-secondary mt-3 mb-3">QUAY LẠI</a>
		...
		<?php
            ...
            $rolePerson = 2;
			$conn = connect_db();
			$sql = "SELECT * from dayOff where rolePerson=? ORDER BY dayApplyOff DESC, idStatus ASC ";
			$stm = $conn->prepare($sql);
            $stm->bind_param("i", $rolePerson); 
            if (!$stm->execute()) {
                return array(
                    'code' => 2,
		...
		?>
		<tr>
			<td class="text-center"><?= $idDayOff ?></td>
			<td><?= $dayApplyOff ?></td>
			<td class="text-center"><?= $idPerson ?></td>
			<td><?= $namePerson ?></td>
            <td><?= $idDepartment ?></td>
			<td class="text-center"><?= $dayOffWant ?></td>
