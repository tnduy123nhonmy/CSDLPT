...
<?php
    ...
    $id = $_GET['resetID'];
    if (isset($_POST['resetID']) && !empty($_POST['resetID'])) {
...
<body>
    ...
    <div class="row justify-content-center">
        <div class="col-md-6 col-lg-5">
            <h3 class="text-center mt-3 mb-3 text-danger font-weight-bold">Reset mật khẩu</h3>
            <form method="POST" action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" class="border rounded w-100 mb-5 mx-auto px-3 pt-3 bg-light">
                <p class="alert alert-danger">Bạn chắc chắn muốn reset mật khẩu nhân viên có mã <?= $_GET['resetID'] ?> không?</p>
                <div class="form-group">
                    <!-- <label for="id">Mã phòng ban:</label> --> 
                    <input type="hidden" name="resetID" class="form-control" value="<?= $_GET['resetID'] ?>">
                </div>
                <div class="form-group text-center">
                    <input type="submit" class="btn btn-danger" value="Reset">
                    <a href="/user/editUser.php?updateID=<?=  $id; ?>" class="btn btn-secondary">Quay lại</a>
                </div>
            </form>
        </div>
    </div>
